class SaveAnswer
  ...
  #新規作成
    ...
    sum_array = [:studyingTime, :answeredQuestionNum, :correctAnswerNum, :studyCountNum, :answeredUnitNum]
  ...
  protected 
    ...
    def set_unit
      ...
      #answeredUnitNum
      if !set_data[drill_id][:units][unit_id][:updated_date] || set_data[drill_id][:units][unit_id][:updated_date].to_date != sent_date.to_date
        set_data[drill_id][:answeredUnitNum][:total] += 1
        set_data[drill_id][:answeredUnitNum][:dailyArr][11][sent_date.to_date.day - 1] += 1
      end
      ...
      #最後のquestion_id
      set_data[drill_id][:units][unit_id][:latestQuestionID] = parameters[:question][parameters[:question].length - 1][:id]
      
      #unitの更新時間
      set_data[drill_id][:units][unit_id][:updated_date] = sent_date
